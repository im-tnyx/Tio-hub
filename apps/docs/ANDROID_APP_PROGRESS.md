...
- [ ] **Exercise Media Storage Apply:** The live `tio-exercise-media` bucket currently drifts from the client contract. Its owner/permanent-user RLS, explicit image/video MIME controls, public playback contract, and 50 MB ceiling are checked into the pending migration but require an explicitly approved Supabase apply.
